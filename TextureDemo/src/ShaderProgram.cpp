...
void ShaderProgram::setUniformSampler(const GLchar* name, const GLuint textureID)
{
	glActiveTexture(GL_TEXTURE0 + textureID);
	GLint loc = getUniformLocation(name);
	glUniform1i(loc, textureID);
}
void ShaderProgram::setUniform(GLuint shaderProgramID, const char* uniformName) {
